...
export function scalarComponent(item: Item, isRequired: boolean, path: Path, key: string | number | undefined, noLabel = false) {
    ...
    if (Schema.hasComplexInput(item)) {
        return <GroupContainer input={item.input} label={label} help={item.help} path={path} key={key} />
        ...
        if (Schema.hasNumericInput(item)) {
            if (item.input === 'int') {
                return <IntInput label={label} help={item.help} path={path} validator={Validators.commonForItem(item)} key={key} />;
            } else if (item.input === 'float') {
                return <FloatInput label={label} help={item.help} path={path} validator={Validators.commonForItem(item)} key={key} />;
            }
            assert(false, `Unhandled numeric input "${item.input}"`);
        } else {
            return <StringInput label={label} help={item.help} path={path} validator={Validators.commonForItem(item)} key={key} />;
        }
        ...
        return <CalendarDateInput label={label} path={path} help={item.help} key={key} />;
    } else if (Schema.hasRelatedToInput(item)) {
        return <RelatedToInput label={label} relatesTo={item.relatesTo} relatedKeys={item.relatedKeys} help={item.help} path={path} key={key} />;
