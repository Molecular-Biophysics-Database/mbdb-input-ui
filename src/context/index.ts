...
import { AutogeneratedItem, CustomItem, Item, Schema, TItem, VariantItem } from '../schema';
                ...
                if (Schema.hasComplexInput(item) || Schema.hasVariantInput(item)) {
                    ...
                    const pusher = Schema.hasComplexInput(item)
                        ? (arrayInnerPath: Path) => {
                            ...
                            Data.set(data, arrayInnerPath, empty);
                        }
                        : (arrayInnerPath: Path) => {
                            const empty = {};
...
export const FormContext = {
    ...
    load(inData: any, ctx: FormContext) {
        ...
        const data = {};
        const references = {};
        this.makeData(ctx.schema, data, references);
        applyInitialData(inData, [], ctx.schema, data, references);

        // TODO: We have to do something about the references.
        // Right now they won't be set to anything...
