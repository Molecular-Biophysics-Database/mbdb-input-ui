...
import { AutogeneratedItem, ComplexInput, CustomItem, Item, RelatedToItem, Schema, TItem, TopLevelItem, VariantItem } from '../schema';
...
function checkAndFixupRelatedToItem(data: DataTree, path: Path, references: ReferenceAnchors, item: RelatedToItem) {
    const v = Data.getValue(data, path);

    if (!Value.isEmpty(v)) {
        const relTo = Value.toRelTo(v);
        if (!References.has(references, item.relatesTo, relTo.id)) {
            throw new Error(`Item on path "${Data.Path.toString(path)}" references a referenceable that does not exist.`);
        }
        ...
        for (const relKey of item.relatedKeys) {
            if (relKey !== 'id') {
                ...
                if (!Value.isValue(rv) || !(Value.isTextual(rv) || Value.isBoolean(rv) || Value.isTristate(rv))) {
                    throw new Error(`Item on path "${Data.Path.toString(path)}" expected its referenceable to have a key "${relKey}" but the referenceable does not have it.`);
                }
    ...
    if (Data.isDataTree(dataItem)) {
        for (const k in dataItem) {
            ...
            if (Schema.hasCustomInput(item)) {
                ...
                if (item.isArray) {
                    const a = Data.getArray(data, innerPath);
                    for (let idx = 0; idx < a.length; idx++) {
                        const elemPath = Data.Path.index(idx, innerPath);
                        checkAndFixupRelatedToItem(data, elemPath, references, item);
                    }
                } else {
                    checkAndFixupRelatedToItem(data, innerPath, references, item);
                }
            } else {
...
export const FormContext = {
    ...
    makeDefaultTrivialData(item: Item, data: DataTree) {
        if (Schema.hasBooleanInput(item)) {
            ...
            iv.payload.tag = dv;
            iv.isValid = Validators.commonForItem(item)(iv.payload as any);
            data[item.tag] = iv;
    ...
    makeTrivialData(item: TItem<any>, data: DataTree, references: ReferenceAnchors) {
        if (item.defaultValue !== undefined) {
            ...
            if (Schema.hasAutogeneratedInput(item)) {
                ...
                // NOTE: Some parts of documentation suggest that internal IDs will be generated by the databases
                //       when a record gets deposited. It, therefore, makes no sense to provide internal IDs in
                //       the data that we submit to the backend. Unforunately, at the time when this code was written
                //       the backend refused any data that did not have internal IDs set.
                //       To work around the problem we just generate the internal IDs here.
                //
                //       We are doing it like this to make in obvious that this is workaround.
                data[item.tag] = Value.textual(Uuid.get(), true);
